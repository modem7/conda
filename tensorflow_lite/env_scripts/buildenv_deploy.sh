...
# CONDAENV="tensorflow"
# PYTHONENV="3.7"
# CONDAREQS="/data/env_scripts/dependencies_deploy.txt"
# #PIPUPGRADE="pip setuptools wheel"
# #PIPREQS="/data/pip-requirements.txt"
# ENVFILE="/data/environment_deploy.yml"
...
mamba create -v --no-deps --name $CONDAENV -y --file $CONDAREQSDEPLOY python=$PYTHONENV &
PID=$!
wait $PID
trap - TERM INT
wait $PID
...
mamba env export --no-builds -n $CONDAENV > $ENVFILEDEPLOY

# Install additional packages that conflict otherwise
#mamba install -y libgfortran5==9.3.0

echo ""
echo "### Created environment file in $ENVFILEDEPLOY"
echo ""
