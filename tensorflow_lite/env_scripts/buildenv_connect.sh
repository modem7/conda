...
# CONDAENV="tensorflow"
# PYTHONENV="3.7"
# CONDAREQS="/data/env_scripts/dependencies_connect.txt"
#PIPUPGRADE="pip setuptools wheel"
#PIPREQS="/data/pip-requirements.txt"
# ENVFILE="/data/environment_connect.yml"
...
mamba create -v --no-deps --name $CONDAENV -y --file $CONDAREQSCONNECT python=$PYTHONENV &
PID=$!
wait $PID
trap - TERM INT
wait $PID
...
mamba env export --no-builds -n $CONDAENV > $ENVFILECONNECT

# Install additional packages that conflict otherwise
#mamba install -y libgfortran5==9.3.0

echo ""
echo "### Created environment file in $ENVFILECONNECT"
echo ""
